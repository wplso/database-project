...
# 4. 출판사명 대소문자 통일
print("출판사명 대소문자 중복 검사 및 통일 작업 중...\n")

    # 현재 존재하는 유니크한 출판사 명 리스트 추출
unique_publishers = df['PUBLISHER_NM'].dropna().unique()

    # 소문자로 변환했을 때 같은 그룹끼리 묶기
    # 예: {'taschen': ['Taschen', 'TASCHEN']}
lower_map = {}
for name in unique_publishers:
    lower_name = str(name).lower()
    if lower_name not in lower_map:
        lower_map[lower_name] = []
    lower_map[lower_name].append(name)

    # 중복이 발생하는 경우(2개 이상) 통일 규칙 생성
rename_dict = {}
for lower_name, original_names in lower_map.items():
    if len(original_names) > 1:
        # 가장 먼저 등장한 표기를 대표값으로 선정
        representative_name = original_names[0] 
        
        # 대표값을 제외한 나머지를 대표 이름으로 변경
        for i, org_name in enumerate(original_names):
            if i > 0: 
                rename_dict[org_name] = representative_name
                print(f"  [통일] '{org_name}' -> '{representative_name}'")

    # 데이터프레임에 적용
if rename_dict:
    df['PUBLISHER_NM'] = df['PUBLISHER_NM'].replace(rename_dict)
    print(f"  -> 총 {len(rename_dict)}개의 출판사명 표기가 통일되었습니다.\n")
else:
    print("  -> 대소문자 중복 출판사가 발견되지 않았습니다.\n")
    


# 5. 값이 있는 행만 추출 (IMAGE_URL 제외)
print(f'{not_null_cols} -> 값이 있는 행만 추출...\n')
df = df.dropna(subset=not_null_cols)


# 6. 글자수 제한 필터링 (max_length 이하) -> length_check_cols 대상
